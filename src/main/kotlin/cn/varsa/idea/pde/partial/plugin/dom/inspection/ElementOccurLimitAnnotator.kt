...
class ElementOccurLimitAnnotator : DomElementsAnnotator {
  override fun annotate(element: DomElement, holder: DomElementAnnotationHolder) {
    ...
    val childrenCounts = DomUtil.getChildrenOfType(element, ExtensionElement::class.java).groupBy { it.xmlElementName }
    ...
    limits.forEach {
      ...
      if (it.minOccurs > 0 && count < it.minOccurs) {
        holder.createProblem(
          element, HighlightSeverity.ERROR, message("inspection.dom.plugin.minSubTag", count, tagName, it.minOccurs)
        )
      }
      if (it.maxOccurs > ExtensionElement.OccursLimit.unbounded && count > it.maxOccurs) {
        holder.createProblem(
          element, HighlightSeverity.ERROR, message("inspection.dom.plugin.maxSubTag", count, tagName, it.maxOccurs)
        )
      }
    }
  }
