...
class PDETargetRemoteRunConfigurationEditor : SettingsEditor<PDETargetRemoteRunConfiguration>(), PanelWithAnchor {
  ...
  enum class JDKVersionItem {
    JDK9 {
      ...
      override fun getLaunchCommandLine(connection: RemoteConnection): String = JDK5to8.getLaunchCommandLine(connection)
    ...
    JDK5to8 {
      ...
      override fun getLaunchCommandLine(connection: RemoteConnection): String =
        connection.launchCommandLine.replace("-Xdebug", "").replace("-Xrunjdwp:", "-agentlib:jdwp=").trim { it <= ' ' }
    },
    JDK1_4 {
