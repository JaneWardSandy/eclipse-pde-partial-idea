...
class VersionRange(
  val leftClosed: Boolean = true, val left: Version, val right: Version? = null, val rightClosed: Boolean = false
) {
  companion object {
  ...
  private val versionRangeString: String by lazy {
    if (right == null) left.toString() else buildString {
  ...
  private val exact: Boolean by lazy {
    if (empty || right == null) false
    else if (leftClosed) {
  ...
  private val empty: Boolean by lazy {
    if (right == null) false else {
      val comparison = left.compareTo(right)
      if (comparison == 0) !leftClosed || !rightClosed else comparison > 0
    }
  }

  fun includes(version: Version): Boolean = if (empty) false
  else if (left.compareTo(version) >= if (leftClosed) 1 else 0) false
  else if (right == null) true
  else right.compareTo(version) >= if (rightClosed) 0 else 1

  fun isEmpty(): Boolean = empty
  fun isExact(): Boolean = exact

  override fun toString(): String = versionRangeString
