...
object ExportPackageParser : BasePackageParser() {
  ...
  override fun getReferences(headerValuePart: HeaderValuePart): Array<PsiReference> {
    ...
    if (parent is ManifestHeaderPart.Clause) {
      ...
      return headerValuePart.node
        .getChildren(tokenFilter)
        .mapNotNull { it as? ManifestToken? }
        .flatMap { getPackageReferences(it).toList() }
        .toTypedArray()
    }
  ...
  override fun checkValuePart(clause: ManifestHeaderPart.Clause, holder: AnnotationHolder): Boolean {
    ...
    if (hostExportPackages != null && packageName !in hostExportPackages && hostExtensibleAPI != true) {
      holder
        .newAnnotation(HighlightSeverity.WARNING, ManifestBundle.message("manifest.lang.additionalAPIToHost"))
  ...
  override fun checkDirectives(clause: ManifestHeaderPart.Clause, holder: AnnotationHolder): Boolean {
    ...
    if (names.count { it == USES_DIRECTIVE } > 1) {
      holder
        .newAnnotation(HighlightSeverity.ERROR, ManifestBundle.message("manifest.lang.duplicate", USES_DIRECTIVE))
    ...
    for (directive in directives) {
      ...
      if (directiveName == X_INTERNAL_DIRECTIVE) {
        ...
        val manifests = if (directiveName == X_FRIENDS_DIRECTIVE) {
          BundleManifestIndex
            .getAllManifestBySymbolicNames(strings, clause.project)
        ...
        for (name in strings) {
          ...
          if (directiveName == USES_DIRECTIVE) {
            val empty = JavaPsiFacade
              .getInstance(clause.project)
              .findPackage(name)
              ?.getDirectories(ProjectScope.getAllScope(clause.project))
              .isNullOrEmpty()
            if (empty) {
              holder.newAnnotation(
            ...
            holder
              .newAnnotation(HighlightSeverity.ERROR, ManifestBundle.message("manifest.lang.bundleNotExists", name))
              .range(range)
              .create()
            annotate = true
          }
