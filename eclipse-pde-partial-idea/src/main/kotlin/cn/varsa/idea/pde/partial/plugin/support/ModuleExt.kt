...
fun Module.isExportedPackageFromRequiredBundle(packageName: String): Boolean {
    ...
    // Bundle required directly
    requiredBundle.mapNotNull { managementService.bundles[it]?.manifest }
    ...
    // Re-export dependency tree can resolve bundle
    allRequiredFromReExport.mapNotNull { managementService.bundles[it]?.manifest }
        .any { it.getExportedPackage(packageName) != null }.ifTrue { return true }
    ...
    }.any { isPackageFromReExportOnly(it, packageName, cacheService, managementService, modulesManifest) }
        .ifTrue { return true }
...
private fun isPackageFromReExportOnly(
    ...
    // Re-export directly
    manifest.reExportRequiredBundleSymbolNames.mapNotNull { managementService.bundles[it]?.manifest }
        .any { it.getExportedPackage(packageName) != null }.ifTrue { return true }
    ...
    // Re-export dependency tree can resolve bundle
    allReExport.mapNotNull { managementService.bundles[it]?.manifest }
        .any { it.getExportedPackage(packageName) != null }.ifTrue { return true }
