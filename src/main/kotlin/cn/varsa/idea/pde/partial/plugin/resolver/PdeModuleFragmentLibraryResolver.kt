...
class PdeModuleFragmentLibraryResolver : ManifestLibraryResolver {
  ...
  override fun postResolve(area: Module) {
    ...
    val moduleMap =
      project.allPDEModules(area).mapNotNull { cacheService.getManifest(it) }.associateBy { it.bundleSymbolicName?.key }
    ...
    area.updateModel { model ->
      ...
      val fragment2HostOrder = fragment2Host.map { (fragment, host) ->
        (orderEntriesMap[fragment] ?: orderEntriesMap["$ProjectLibraryNamePrefix$fragment"]) to (orderEntriesMap[host]
